...
func rotate_only_head(mouse_axis : Vector2) -> void:
	head.rotate_only_camera(mouse_axis)
...
func move(_delta: float, input_axis := Vector2.ZERO, input_jump := false, input_crouch := false, input_sprint := false, move_multiplier := 1.0):
	if is_fly_mode() or is_floating():
		_direction_base_node = head
	else:
		_direction_base_node = self
	super.move(_delta, input_axis, input_jump, input_crouch, input_sprint, move_multiplier)
