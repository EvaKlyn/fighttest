...
@export_group("Networked Gameplay Values")
@export var hit_stunned := false
@export var hit_stun_time := 0.0
...
## Ability that allows dodging when pushing sprint and jump
@onready var dodge_ability: DodgeAbility3D = get_node(NodePath("DodgeAbility3D"))

## Ability that allows for knockback
@onready var knockback: KnockBackAbility3D = get_node(NodePath("KnockbackAbility"))
...
# true for the moment the player is hit
var is_hit := false
# true if the player has already bounced this airtime
var wall_bounced := false
# true if the player is in iframes
var dodge := false
...
func move(_delta: float, input_axis := Vector2.ZERO, input_jump := false, input_crouch := false, input_sprint := false, move_multiplier := 1.0) -> void:
	...
	if hit_stunned and not is_on_floor() and is_on_wall() and not wall_bounced:
		...
		velocity.z = bounced_velocity.z * 1.5
		add_hit_stun(0.2)
		wall_bounced = true
	...
	attack_state = get_most_relevant_state()


func get_most_relevant_state() -> String:
	if hit_stunned:
		return "stun"
	if not is_on_floor():
		return "air"
	if dodge_ability.is_actived():
		return "dodge"
	if crouch_ability.is_actived():
		return "crouch"
	if sprint_ability.is_actived():
		return "sprint"
	if walk_ability.is_actived():
...
func _connect_signals():
	...
	dodge_ability.actived.connect(_on_jumped.bind())
